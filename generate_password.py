...
def build_password(length, use_uppercase, use_lowercase, use_digits, use_specials):
    if length <= 0:
    ...
    if remaining_length < 0:
        raise ValueError("Password length is too short for selected character types.")
...
def copy_to_clipboard():
    ...
    if password:
        if pyperclip is None:
            messagebox.showerror("Missing Dependency", "Install pyperclip to use clipboard copy.")
            return
...
def create_app():
    ...
    global root, length_entry, uppercase_var, lowercase_var, digits_var, specials_var, password_output
